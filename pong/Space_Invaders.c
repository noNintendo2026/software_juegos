...
void draw_saucer() {
    ...
    if (saucer.alive == 1) {
        //use a local dest rect so SDL doesn't modify the hitbox when clipping
        dest.x = saucer.hitbox.x;
        dest.y = saucer.hitbox.y;
        dest.w = saucer.hitbox.w;
        dest.h = saucer.hitbox.h;

        SDL_BlitSurface(saucer_img, &src, screen, &dest);
    }
}
...
void draw_bases() {
    ...
    for (i = 0; i < BASE; i++) {
        //use a local dest rect so SDL doesn't modify the hitbox when clipping
        dest.x = base[i].hitbox.x;
        dest.y = base[i].hitbox.y;
        dest.w = base[i].hitbox.w;
        dest.h = base[i].hitbox.h;

        SDL_BlitSurface(base_img[i], &src, screen, &dest);
    }
}
...
void draw_player() {
    ...
    //use a local dest rect so SDL doesn't modify the hitbox when clipping
    dest.x = player.hitbox.x;
    dest.y = player.hitbox.y;
    dest.w = player.hitbox.w;
    dest.h = player.hitbox.h;

    SDL_BlitSurface(player_img, &src, screen, &dest);
}
...
int load_image(char filename[], SDL_Surface** surface, enum ck_t colour_key) {
    ...
    if (colour_key == magenta) {
        ...
        colourkey = SDL_MapRGB(temp->format, 0, 0, 0);
    }
